...
The timestamp-noise model is applied only to reception timestamps. Transmission timestamps are not perturbed by the timestamp-jitter random variable.

A new timestamp-noise sample is generated independently for every successfully received synchronization packet.

## 2. Timestamp Jitter

Timestamp jitter is modeled as a zero-mean Gaussian random variable:
...
The default timestamp-jitter standard deviation is
...
The default velocity-error standard deviation is

$$\sigma_v=0.2\ \mathrm{m/s}$$

A new velocity-error sample is generated when a protocol obtains a radial-velocity measurement associated with a received packet.
...
Timestamp, velocity, and position errors are generated using separate random-variable streams.
...
* velocity-error samples generated for different measurements,
* horizontal and vertical position-error samples,
* and noise samples generated in different Monte Carlo runs.

The explicit stream assignments and run-number configuration are documented in `docs/random-streams.md`.

## 8. Units

| Quantity                  | Internal unit              | Reporting unit               |
| ------------------------- | -------------------------- | ---------------------------- |
| Timestamp jitter          | Seconds                    | Microseconds                 |
| Clock offset              | Seconds                    | Microseconds or milliseconds |
...
Clock-skew error in parts per million is calculated as

$$e_{\mathrm{skew,ppm}} = \left| \hat{\alpha}-\alpha\right|\times10^6$$

where $\alpha$ is the true clock-skew scale factor and $\hat{\alpha}$ is its estimate.
