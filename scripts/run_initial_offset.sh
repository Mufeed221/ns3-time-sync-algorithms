...
VALUES=(0.02 0.05 1 5 10 20)
PROTOCOLS=("LW-Sync" "LT-Sync" "DC-Sync")
TRACKS=(1 2)

OUTPUT_FILE="$RAW_DIR/initial_offset_long.csv"
STATS_FILE="$PROCESSED_DIR/initial_offset_stats.csv"
...
echo "Run,ClockOffset,Track,Protocol,OffsetErrorUs" > "$OUTPUT_FILE"
...
echo "========================================="
echo "Initial-offset experiment"
...
echo "ClockOffset,T1_LW_Avg,T1_LW_Std,T1_LT_Avg,T1_LT_Std,T1_DC_Avg,T1_DC_Std,T2_LW_Avg,T2_LW_Std,T2_LT_Avg,T2_LT_Std,T2_DC_Avg,T2_DC_Std" > "$STATS_FILE"
awk -F',' '
BEGIN { OFS="," }
NR==1 { next }
NF>=5 {
