...
OUTPUT_FILE="$RAW_DIR/starting_angle_long.csv"
STATS_FILE="$PROCESSED_DIR/starting_angle_stats.csv"
...
echo "Run,Angle,Track,Protocol,OffsetErrorUs" > "$OUTPUT_FILE"
...
echo "========================================="
echo "Starting-angle experiment"
...
echo "Angle,LW_Avg,LW_Std,LT_Avg,LT_Std,DC_Avg,DC_Std" \
    > "$STATS_FILE"
...
END {
    for (angle in angles) {
        lw = angle SUBSEP "LW-Sync"
        lt = angle SUBSEP "LT-Sync"
        dc = angle SUBSEP "DC-Sync"
