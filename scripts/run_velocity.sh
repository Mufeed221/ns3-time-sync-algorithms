...
OUTPUT_FILE="$RAW_DIR/velocity_long.csv"
STATS_FILE="$PROCESSED_DIR/velocity_stats.csv"
...
echo "Run,InitialVelocity,Track,Protocol,OffsetErrorUs" > "$OUTPUT_FILE"
...
echo "Init. Radial Velocity,LW_Avg,LW_Std,LT_Avg,LT_Std,DC_Avg,DC_Std" \
    > "$STATS_FILE"
...
END {
    for (velocity in velocities) {
        lw = velocity SUBSEP "LW-Sync"
        lt = velocity SUBSEP "LT-Sync"
        dc = velocity SUBSEP "DC-Sync"
