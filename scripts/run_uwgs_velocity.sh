...
OUTPUT_FILE="$RAW_DIR/UWGS_velocity_long.csv"
OFFSET_STATS="$PROCESSED_DIR/UWGS_velocity_(offset)_stats.csv"
SKEW_STATS="$PROCESSED_DIR/UWGS_velocity_(skew)_stats.csv"

[[ -x "$LW_BINARY" ]] || { echo "Missing executable: $LW_BINARY"; exit 1; }
[[ -x "$UWGS_BINARY" ]] || { echo "Missing executable: $UWGS_BINARY"; exit 1; }

echo "Run,Velocity,Protocol,Metric,Value" > "$OUTPUT_FILE"
...
run_one()
{
    ...
    if [[ "$protocol" == "LW-Sync" ]]; then
        binary="$LW_BINARY"
        "$binary" --run="$run_number" --initVelocity="$parameter" --plotNum=1 \
...
calculate_stats()
{
    local metric="$1"
    local output="$2"
    echo "Velocity,LW_Avg,LW_Std,UWGS_Avg,UWGS_Std" > "$output"
