...
VALUES=(10e-6 20e-6 30e-6 40e-6 50e-6)
PROTOCOLS=("LW-Sync" "LT-Sync" "DC-Sync")
TRACKS=(1 2)

OUTPUT_FILE="$RAW_DIR/jitter_noise_long.csv"
STATS_FILE="$PROCESSED_DIR/jitter_noise_stats.csv"
...
echo "Run,Jitter,Track,Protocol,OffsetErrorUs" > "$OUTPUT_FILE"
...
echo "========================================="
echo "Timestamp-jitter experiment"
...
echo "Jitter,T1_LW_Avg,T1_LW_Std,T1_LT_Avg,T1_LT_Std,T1_DC_Avg,T1_DC_Std,T2_LW_Avg,T2_LW_Std,T2_LT_Avg,T2_LT_Std,T2_DC_Avg,T2_DC_Std" > "$STATS_FILE"
awk -F',' '
BEGIN { OFS="," }
NR==1 { next }
NF>=5 {
