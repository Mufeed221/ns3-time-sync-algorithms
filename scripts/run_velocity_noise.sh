...
OUTPUT_FILE="$RAW_DIR/velocity_noise_long.csv"
STATS_FILE="$PROCESSED_DIR/velocity_noise_stats.csv"
...
echo "Run,VelocityNoise,Track,Protocol,OffsetErrorUs" > "$OUTPUT_FILE"
...
echo "========================================="
echo "Velocity-noise experiment"
...
echo "Vel_Noise,T1_LW_Avg,T1_LW_Std,T1_LT_Avg,T1_LT_Std,T1_DC_Avg,T1_DC_Std,T2_LW_Avg,T2_LW_Std,T2_LT_Avg,T2_LT_Std,T2_DC_Avg,T2_DC_Std" > "$STATS_FILE"
awk -F',' '
BEGIN { OFS="," }
NR==1 { next }
NF>=5 {
